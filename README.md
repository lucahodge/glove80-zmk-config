...
- configuration is in `config/glove80.map`
- use [https://nickcoutsos.github.io/keymap-editor/]
  - use `clipboard` mode
  - copy and paste configuration into editor
  - then copy and paste back into file
- commit and push changes, GitHub actions will produce `.utf` file
- flash firmware
  - one at a time for each half (right half first)
  - put into boot mode, plug into computer
    - right side: turn off, hold 'bottom right key' and 'i' down, turn on
    - left side: turn off, hold 'bottom left key' and 'e' down, turn on
  - drag `utf` file into the drive
